...
fixBinaryReprFuckup :: P.ByteString -> P.ByteString
fixBinaryReprFuckup = BS.pack . VU.toList . btsToVector
  where btsToVector :: P.ByteString -> VU.Vector Word8
        btsToVector bts = let bs = BS.filter (\x -> 32 /= x && 10 /= x && 13 /= x) bts
                  in VU.generate (fromIntegral (BS.length bs) `div` 2) $ \i -> 16 * adjustAscii (bs `BS.index` (fromIntegral (2*i))) + adjustAscii (bs `BS.index` (fromIntegral (2*i+1)))
